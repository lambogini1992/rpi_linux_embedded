...
QUEUE_PACK mq_server[ROUTING_MAX];
...
int main(int argc, char **argv)
{
	...
	{
		ret_val = open_queue_server(&mq_server[index], queue_name[index] \
	...
	while(1)
	{
		for(index = 0; index < ROUTING_MAX; index++)
		{
			printf("Msg of %s routing\n", queue_name[index]);
			ret_val = read_queue(&mq_server[index], msg_buff);
			if(ret_val == 0)
			{
				printf("No of msg in queue: %d\n", (int)mq_server[index].attr.mq_curmsgs);
				printf("%s\n", msg_buff);
			}
		}
	...
	for(index = 0; index < ROUTING_MAX; index++)
	{
		ret_val = release_queue(&mq_server[index], queue_name[index]);
		if(ret_val < 0)
		{
			return -1;
		}
